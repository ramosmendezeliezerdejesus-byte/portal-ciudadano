  ...
  return (
    <div style={{ minWidth: 220, maxWidth: 260 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
        <span style={{ background: "#c4714d22", color: "#c4714d", fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 99 }}>
          Denuncia
        </span>
      ...
      <p style={{ fontWeight: 700, fontSize: 13, margin: "0 0 4px", color: "#2d3e50" }}>{report.title}</p>
      {report.description && (
      ...
      <div className="card-soft rounded-2xl shadow-soft border border-brand-teal/10 overflow-hidden">
        <div className="p-5 border-b border-brand-teal/10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Resumen de denuncias ubicadas en el mapa</p>
          </div>
        </div>
        ...
        <div className="p-5 grid grid-cols-2 gap-3">
          <div className="rounded-2xl border border-brand-teal/10 bg-brand-cream dark:bg-brand-navy/30 p-4">
            ...
            <p className="text-xs text-gray-500 mt-1">denuncias con coordenadas</p>
          </div>
          ...
          <div className="rounded-2xl border border-brand-terracotta/20 bg-brand-terracotta/10 p-4 col-span-2">
            <p className="text-[11px] uppercase tracking-wide text-gray-500 font-semibold">Zona con mas denuncias</p>
            <div className="flex items-start justify-between gap-3 mt-2">
              <div>
                ...
                <p className="text-sm text-brand-terracotta font-semibold mt-1">
                  {summary.leadingZone?.count ?? 0} denuncia{summary.leadingZone?.count === 1 ? "" : "s"}
                </p>
              </div>
              {hotspot && (
                <button
      ...
      <div className="card-soft rounded-2xl shadow-soft border border-brand-teal/10 overflow-hidden">
        <div className="p-5 border-b border-brand-teal/10 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">Ranking de ubicaciones con mas denuncias</p>
          </div>
        </div>
              ...
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 70 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [`${value} denuncia${value === 1 ? "" : "s"}`, "Incidencias"]}
              ...
              <div className="grid grid-cols-3 gap-3 mt-4">
                {chartData.slice(0, 3).map((item, index) => (
                  <div key={item.name} className={`p-3 rounded-xl text-center border ${
                    ...
                    <p className="text-[10px] text-gray-400">denuncia{item.count === 1 ? "" : "s"}</p>
                  </div>
                ))}
              </div>
            </>
      ...
      <div className="grid gap-4 xl:grid-cols-1">
        <div className="card-soft rounded-2xl shadow-soft border border-brand-teal/10 overflow-hidden">
          <div className="p-5 border-b border-brand-teal/10">
            ...
            <p className="text-xs text-gray-400 mt-1">Que tipo de denuncia domina en el territorio</p>
          </div>
          <div className="p-5">
            {categoryChartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  ...
                  <Tooltip formatter={(value) => [`${value} denuncia${value === 1 ? "" : "s"}`, "Categoria"]} />
...
export default function ZonasDenuncias() {
  const { token } = useAuth();
  ...
  const fetchProposals = useCallback(async () => {
    ...
    try {
      ...
      const reportsRes = await fetch(`${API}/denuncias?${params.toString()}`, { headers });
      const reportsData = await reportsRes.json();
      if (!reportsRes.ok) throw new Error(reportsData.error || "No se pudieron cargar las denuncias");

      setProposals((reportsData.reports ?? []).map((report) => ({ ...report, case_type: "denuncia" })).filter((report) => report.latitude && report.longitude));
    } catch (err) {
      ...
      <header className="fixed top-0 left-0 right-0 z-[1000] card-soft shadow-soft">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif font-bold text-brand-navy dark:text-brand-cream text-base leading-none">Zonas de Denuncias</h1>
              <p className="text-xs text-gray-400">{proposals.length} denuncias geolocalizadas</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <NavMenu currentPath="/zonas-denuncias" />
          </div>
        </div>
      </header>
          ...
          <aside className="w-80 shrink-0 card-soft border-r border-brand-teal/10 overflow-y-auto scrollbar-hide z-[998]">
            <div className="p-4 border-b border-brand-teal/10">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">
                {proposals.length} denuncias con GPS
              </p>
            </div>
            <div className="divide-y divide-brand-teal/5">
              {proposals.length === 0 ? (
                <div className="p-8 text-center text-sm text-gray-400">
                  No hay denuncias con coordenadas disponibles.
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-brand-terracotta/10 text-brand-terracotta">
                            Denuncia
                          </span>
          ...
          {proposals.length === 0 && !loading && (
            <div className="absolute inset-0 z-[996] flex items-center justify-center pointer-events-none">
              <div className="card-soft rounded-2xl p-8 text-center border border-brand-teal/10 shadow-soft max-w-xs mx-4">
                ...
                <p className="text-sm text-gray-400">Las denuncias con ubicacion GPS apareceran aqui.</p>
              </div>
            </div>
          )}
